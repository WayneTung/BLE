  ...
  if ( keys & HAL_KEY_UP )
  {
              ...
              simpleBLEProcedureInProgress == FALSE )
    {
      ...
      if ( simpleBLEDoWrite )
      {
		...
		req.len = 5;
		req.pValue = "0123456";
		//req.pValue = osal_msg_allocate(8);
		//osal_memcpy(req.pValue,"12345678",8);
		status = GATT_WriteLongCharValue( simpleBLEConnHandle, &req, simpleBLETaskId );
		HalLcdWriteString(req.pValue,HAL_LCD_LINE_1);
...
static void simpleBLECentralProcessGATTMsg( gattMsgEvent_t *pMsg )
{
  ...
  tmp[0]= pMsg->method;
  //tmp[1]=',';
  tmp[1]= pMsg->msg.errorRsp.reqOpcode;
  //tmp[3]='\n';
  sbpSerialAppWrite(tmp,2);
#endif
...
uint8 sbpGattWriteString(uint8 *pBuffer, uint16 length)
{
	...
	osal_memcpy(req.pValue,pBuffer,len);
	status = GATT_WriteLongCharValue( simpleBLEConnHandle, &req, simpleBLETaskId );
	osal_msg_deallocate(req.pValue);
        return status;
}
